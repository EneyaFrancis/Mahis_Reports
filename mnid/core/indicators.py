...
def _enrich_program_based_mnid_indicators(indicators: list, categories: list[str] | None = None, include_moh: bool = False) -> list[dict]:
    ...
    fallback_list = list(_program_based_priority_indicators(categories))
    if include_moh:
        # MoH's own indicator set now lives in its own file (mnid/dashboards/
        # MNH-MoH/indicators.py), same as MNH-Nest360's -- no longer defined
        # inline here, so pull it in explicitly when a MoH-role view asks
        # for it instead of leaving it permanently filtered out.
        try:
            ...
            if get_mnh_moh_indicators is None:
                ...
                _ipath = _pp(__file__).resolve().parents[1] / 'dashboards' / 'MNH-MoH' / 'indicators.py'
            ...
            fallback_list.extend(get_mnh_moh_indicators())
        except Exception:
            _LOGGER.exception('Failed to load MNH-MoH indicators for include_moh=True')
